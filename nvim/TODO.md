...
* **diagnostic status line:** The current line diagnostic in lualine gets truncated from the left, which makes it hard to read. Figure out 1) how to truncate from the right, or 2) how to display it somewhere else - maybe in a single-line floating window just above the statusline (how does fidget.nvim's display work?)
